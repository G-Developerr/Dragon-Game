...
function updateShopDisplay() {
    ...
    items.forEach((item) => {
        ...
        item.innerHTML = `
            <div class="shop-item-icon ${itemId.replace("_", "-")}-icon"></div>
...
function equipItem(itemId, category) {
    equippedItems[category] = equippedItems[category] === itemId ? null : itemId;
    updateShopDisplay();
...
function sellLastItem() {
    ...
    if (equippedItems.glasses === itemId) equippedItems.glasses = null;
...
// ENHANCED DRAGON WITH WINGS AND ACCESSORIES - ΔΙΟΡΘΩΜΕΝΟ!
function drawDragon() {
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ...
    dragon.segments.forEach((segment, index) => {
        ...
        if (index === 2) {
            ...
            wingGradient.addColorStop(0, dragon.customColor || "#ff6b6b");
            wingGradient.addColorStop(0.4, `${dragon.customColor || "#ff6b6b"}DD`);
            wingGradient.addColorStop(0.8, `${dragon.customColor || "#8b0000"}BB`);
            wingGradient.addColorStop(1, `${dragon.customColor || "#5a0000"}99`);
            ...
            wingGradient2.addColorStop(0, dragon.customColor || "#ff6b6b");
            wingGradient2.addColorStop(0.4, `${dragon.customColor || "#ff6b6b"}DD`);
            wingGradient2.addColorStop(0.8, `${dragon.customColor || "#8b0000"}BB`);
            wingGradient2.addColorStop(1, `${dragon.customColor || "#5a0000"}99`);
        ...
        if (isHead) {
            ...
            // ✅ DRAW GLASSES HERE - ΣΩΣΤΑ ΤΟΠΟΘΕΤΗΜΕΝΑ!
            ctx.save();
            ctx.translate(segment.x, segment.y);
            ctx.rotate(segment.rotation);
            ...
            if (equippedItems.glasses) {
                ...
                const glassesOffset = headSize * 0.3; // Μπροστά από τα μάτια
...
console.log("Enhanced Dragon Game with Complete Shop System loaded successfully!");